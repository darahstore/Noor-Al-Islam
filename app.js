...
// ==========================================
const sebhaClickSound = new Audio('sebha-click.mp3'); // ملف صوت المسبحة
const adhanAudio = new Audio('adhan.mp3');            // ملف صوت الأذان
let isAdhanEnabled = true;                            // حالة تشغيل الأذان
let lastAdhanPlayedName = "";                         // لتجنب تكرار الأذان لنفس الصلاة

// 1. فك حظر الصوت في الهواتف والـ WebView (Audio Unlocker)
...
const StorageManager = {
    state: {
        ...
        soundVib: true,
        sebhaTheme: 'default',
        sebhaSize: 1, // الحجم الافتراضي للمسبحة
        lastSurahMushaf: 1,
        country: '',
        city: '',
        isProUser: false
...
// ==========================================
// دالة تطبيق حجم المسبحة الرقمية
// ==========================================
function applySebhaSize(size) {
    const tallyCounter = document.getElementById('digital-tally-counter');
    if (tallyCounter) {
        const numericSize = parseFloat(size) || 1;
        tallyCounter.style.transform = `scale(${numericSize})`;
        tallyCounter.style.transformOrigin = 'center center';
        tallyCounter.style.transition = 'transform 0.2s ease-in-out';
    }
}
...
function applyLanguage(lang) {
    ...
    const navAudio = document.querySelector('#nav-audio span:last-child');
    if (navAudio) navAudio.innerText = dict.audioTab;
    
    const navMushaf = document.querySelector('#nav-mushaf span:last-child');
    if (navMushaf) navMushaf.innerText = dict.mushafTab;
    
    const navSebha = document.querySelector('#nav-sebha span:last-child');
    if (navSebha) navSebha.innerText = dict.sebhaTab;
    
    const navPrayer = document.querySelector('#nav-prayer span:last-child');
    if (navPrayer) navPrayer.innerText = dict.prayerTab;
    ...
    async function fetchSurahText(surahId) {
        ...
        try {
            ...
            data.data.ayahs.forEach(ayah => {
                let text = ayah.text;
                // حذف البسملة من أول الآية إذا لم تكن الفاتحة أو التوبة
            ...
            console.error("خطأ جلب نص السورة:", err);
...
function initPrayerEngine() {
    ...
    // زر التحكم في صوت الأذان
    const prayerTabContainer = document.getElementById('tab-prayer');
    if (prayerTabContainer && !document.getElementById('adhan-volume-ctrl')) {
        const adhanCtrlBtn = document.createElement('button');
        adhanCtrlBtn.id = 'adhan-volume-ctrl';
        adhanCtrlBtn.className = 'w-full mt-4 py-3 bg-blue-600 text-white rounded-lg font-bold shadow-md';
        adhanCtrlBtn.innerText = isAdhanEnabled ? '🔊 صوت الأذان: مفعل' : '🔇 صوت الأذان: معطل';
        adhanCtrlBtn.onclick = () => {
            isAdhanEnabled = !isAdhanEnabled;
            adhanAudio.muted = !isAdhanEnabled;
            adhanCtrlBtn.innerText = isAdhanEnabled ? '🔊 صوت الأذان: مفعل' : '🔇 صوت الأذان: معطل';
            if (isAdhanEnabled) { 
                adhanAudio.play().then(() => adhanAudio.pause()).catch(() => {}); 
            }
        };
        prayerTabContainer.appendChild(adhanCtrlBtn);
    }
}

// دالة تحديث القبلة والبوصلة
function updateQiblaCompass(qiblaAngle) {
    ...
    const handleOrientation = (event) => {
        ...
        if (event.webkitCompassHeading) {
            alpha = event.webkitCompassHeading; // iOS support
        }
        if (alpha !== null && alpha !== undefined) {
            let compassRotation = currentQiblaAngle - alpha;
            qiblaCompass.style.transform = `rotate(${compassRotation}deg)`;
        }
    };

    if (window.DeviceOrientationEvent && !isOrientationListenerAdded) {
        if (typeof DeviceOrientationEvent.requestPermission === 'function') {
            qiblaCompass.onclick = () => {
                DeviceOrientationEvent.requestPermission()
                    .then(permissionState => {
                        if (permissionState === 'granted') {
                            window.addEventListener('deviceorientation', handleOrientation, true);
                            isOrientationListenerAdded = true;
                            if (qiblaText) qiblaText.innerText = `زاوية القبلة: ${currentQiblaAngle.toFixed(2)} درجة (تم تفعيل المستشعر)`;
                        }
                    }).catch(console.error);
            };
            if (qiblaText) qiblaText.innerText = `زاوية القبلة: ${qiblaAngle.toFixed(2)} درجة (اضغط على البوصلة لتفعيلها)`;
        } else {
...
async function fetchPrayerTimesByCity(city, country) {
    try {
        ...
        if (data.data) {
            ...
            if (data.data.meta && data.data.meta.qibla !== undefined) {
                updateQiblaCompass(data.data.meta.qibla);
            }
        }
...
function updatePrayerUI(timings) {
    const elFajr = document.getElementById('time-fajr');
    const elDhuhr = document.getElementById('time-dhuhr');
    const elAsr = document.getElementById('time-asr');
    const elMaghrib = document.getElementById('time-maghrib');
    const elIsha = document.getElementById('time-isha');

    if (elFajr) elFajr.innerText = timings.Fajr;
    if (elDhuhr) elDhuhr.innerText = timings.Dhuhr;
    if (elAsr) elAsr.innerText = timings.Asr;
    if (elMaghrib) elMaghrib.innerText = timings.Maghrib;
    if (elIsha) elIsha.innerText = timings.Isha;
...
function startCountdown(timings) {
    ...
    countdownInterval = setInterval(() => {
        ...
        // البحث عن الصلاة القادمة (بما في ذلك الصلاة التي حان وقتها للتو)
        ...
        const secs = (60 - currentSecs) % 60;

        // تشغيل الأذان بدقة عند انطباق الوقت بالدقيقة والثانية 0
        if (diffMinutes === 0 && currentSecs === 0) { 
            if (lastAdhanPlayedName !== nextPrayer.name && isAdhanEnabled) {
                adhanAudio.currentTime = 0;
                adhanAudio.play().catch(e => console.log('تعذر تشغيل الأذان:', e));
                lastAdhanPlayedName = nextPrayer.name;
            }
        }
...
function initAudioEngine() {
    ...
    fetch('https://mp3quran.net/api/v3/reciters?language=ar')
        ...
        }).catch(err => console.error("خطأ تحميل القراء:", err));
    ...
    selectSurah.addEventListener('change', (e) => {
        ...
        if (currentAudioUrl) {
            ...
            }).catch(err => console.log("منع التشغيل التلقائي للصوت:", err));
        }
    });
...
function initSebhaEngine() {
    ...
    if (lcdDisplay) lcdDisplay.innerText = String(sebhaCount).padStart(4, '0');
    ...
    const nextBtn = document.getElementById('btn-next-dhikr');
    if (nextBtn) {
        nextBtn.addEventListener('click', () => {
            currentDhikrIdx = (currentDhikrIdx + 1) % DhikrList.length;
            if (displayText) displayText.innerText = DhikrList[currentDhikrIdx];
            sebhaCount = 0; 
            if (lcdDisplay) lcdDisplay.innerText = "0000";
        });
    }

    const prevBtn = document.getElementById('btn-prev-dhikr');
    if (prevBtn) {
...
function applySebhaTheme(theme) {
    ...
    sebhaSection.classList.remove(
        ...
        'sebha-theme-wood',
        'sebha-theme-marble',
        'sebha-theme-turquoise'
    );
    
    sebhaSection.classList.add(`sebha-theme-${theme}`);
}
...
function initNavigationAndSettings() {
    ...
    if (settingsLang) {
        settingsLang.value = StorageManager.state.lang;
        settingsLang.addEventListener('change', (e) => applyLanguage(e.target.value));
    }
    ...
    // === ربط قائمة حجم المسبحة الإعدادات ===
    const sebhaSizeSelect = document.getElementById('settings-sebha-size') || document.getElementById('select-sebha-size');
    if (sebhaSizeSelect) {
        sebhaSizeSelect.value = StorageManager.state.sebhaSize || 1;
        sebhaSizeSelect.addEventListener('change', (e) => {
            const newSize = e.target.value;
            StorageManager.update('sebhaSize', newSize);
            applySebhaSize(newSize);
        });
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    applyFontSize();
    applySebhaSize(StorageManager.state.sebhaSize || 1); // تطبيق حجم المسبحة فور التحميل
