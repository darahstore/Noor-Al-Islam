...
const StorageManager = {
    state: {
        ...
        soundVib: true,
        sebhaTheme: 'default',
        sebhaSize: 'md', // القيمة الافتراضية النصية للحجم
        lastSurahMushaf: 1,
        country: '',
        city: '',
        isProUser: false
...
// ==========================================
// دالة تطبيق حجم المسبحة الرقمية (النسخة النهائية المصححة)
// ==========================================
function applySebhaSize(sizeValue) {
    // خريطة تحويل القيم النصية من القائمة إلى نسب تكبير رقمية دقيقة
    const sizeMap = {
        'xs': 0.75, // صغير جداً
        'sm': 0.88, // صغير
        'md': 1.0,  // متوسط (افتراضي)
        'lg': 1.2   // كبير
    };

    // استخراج القيمة الرقمية بناءً على الاختيار أو الاعتماد على القيمة المدخلة مباشرة
    const numericSize = sizeMap[sizeValue] || parseFloat(sizeValue) || 1.0;
    
    // استهداف الحاوية الكبرى للمسبحة الرقمية بالكامل لضمان تكبيرها وتصغيرها بسلاسة
    ...
    if (mainContainer) {
        ...
        // بديل احتياطي في حال عدم توفر الحاوية الكبرى
        const tallyBtn = document.getElementById('tally-btn');
        const lcdDisplay = document.getElementById('sebha-count-display');
        
        if (tallyBtn) {
            tallyBtn.style.transform = `scale(${numericSize})`;
            tallyBtn.style.transformOrigin = 'center center';
            tallyBtn.style.transition = 'transform 0.2s ease-in-out';
        }
        if (lcdDisplay) {
            lcdDisplay.style.transform = `scale(${numericSize})`;
            lcdDisplay.style.transformOrigin = 'center center';
            lcdDisplay.style.transition = 'transform 0.2s ease-in-out';
        }
    }
}
...
function updateQiblaCompass(qiblaAngle) {
    ...
    if (window.DeviceOrientationEvent && !isOrientationListenerAdded) {
        if (typeof DeviceOrientationEvent.requestPermission === 'function') {
            ...
            if (qiblaText) qiblaText.innerText = `زاوية القبلة: ${currentQiblaAngle.toFixed(2)} درجة (اضغط على البوصلة لتفعيلها)`;
        } else {
...
function initNavigationAndSettings() {
    ...
    // === ربط قائمة حجم المسبحة الإعدادات (النسخة المُصلحة) ===
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    applyFontSize();
    applySebhaSize(StorageManager.state.sebhaSize || 'md');
