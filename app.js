...
// تم إصلاحه: مشكلة عدم ظهور أسماء السور بسبب فشل مطابقة id القارئ (نص/رقم)
// ==========================================
let allReciters = [];
let currentAudioUrl = "";
...
function initAudioEngine() {
    ...
    selectReciter.addEventListener('change', (e) => {
        ...
        // --- الإصلاح: مقارنة id كنص لتجنب عدم التطابق (String === String) ---
        const reciter = allReciters.find(r => String(r.id) === recId);
        if (!reciter || !reciter.moshaf) {
            console.error("لم يتم العثور على القارئ أو لا يحتوي على روايات");
            return;
        }
        // ---------------------------------------------------------------
    ...
    selectRiwaya.addEventListener('change', (e) => {
        ...
        if(moshafIdx === "" || !recId) return;

        // --- الإصلاح: مقارنة id كنص مع فحص أمان للقارئ والمصحف ---
        const reciter = allReciters.find(r => String(r.id) === recId);
        if (!reciter || !reciter.moshaf || !reciter.moshaf[moshafIdx]) {
            console.error("لم يتم العثور على بيانات المصحف المختار");
            return;
        }
        const moshaf = reciter.moshaf[moshafIdx];
        // -------------------------------------------------------
        
        // --- فحص إضافي: التأكد من وجود surah_list قبل المعالجة ---
        if (!moshaf.surah_list) {
            console.error("المصحف لا يحتوي على قائمة سور (surah_list)");
            return;
        }
        // ---------------------------------------------------------
        ...
        suwarList.forEach(surahNum => {
            ...
            // التأكد من أن الفهرس ضمن نطاق أسماء السور
