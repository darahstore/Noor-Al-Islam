// ==========================================
// === إضافة جديدة: المتغيرات الصوتية العامة ===
// ==========================================
...
function initPrayerEngine() {
    ...
    // === إضافة جديدة: زر التحكم في صوت الأذان ===
...
async function fetchPrayerTimesByCoords(lat, lng) {
    try {
        ...
        if(data.data.meta && data.data.meta.qibla) {
            const qiblaAngle = data.data.meta.qibla;
            qiblaCompass.style.transform = `rotate(${qiblaAngle}deg)`;
            qiblaText.innerText = `زاوية القبلة: ${qiblaAngle.toFixed(2)} درجة`;
            
            // === إضافة جديدة: ربط مستشعرات الاتجاه لتشغيل سهم القبلة ===
            if (window.DeviceOrientationEvent) {
                ...
                // طلب صلاحية المستشعر لأجهزة iOS 13+
                        ...
                        DeviceOrientationEvent.requestPermission()
                            .then(permissionState => {
                                if (permissionState === 'granted') {
                                    ...
                                    qiblaText.innerText = `زاوية القبلة: ${qiblaAngle.toFixed(2)} درجة (تم تفعيل المستشعر)`;
                                }
                            }).catch(console.error);
                    }, { once: true });
                    qiblaText.innerText = `زاوية القبلة: ${qiblaAngle.toFixed(2)} درجة (اضغط على البوصلة لتفعيلها في iOS)`;
                } else {
...
function startCountdown(timings) {
    ...
    countdownInterval = setInterval(() => {
        ...
        // === إضافة جديدة: تشغيل الأذان عند دخول وقت الصلاة ===
        if (hrs === 0 && mins === 0 && secs === 59) { 
            if (lastAdhanPlayedName !== nextPrayer.name && isAdhanEnabled) {
                adhanAudio.currentTime = 0;
                adhanAudio.play().catch(e => console.log('تعذر تشغيل الأذان، قد يحتاج لتفاعل المستخدم المسبق:', e));
...
function initSebhaEngine() {
    ...
    tallyBtn.addEventListener('click', () => {
        ...
        // === إضافة جديدة: تشغيل الصوت عند الضغط على المسبحة ===
        if (StorageManager.state.soundVib) { // تعمل تبعاً لإعدادات صوت التطبيق الحالية
