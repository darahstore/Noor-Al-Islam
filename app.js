...
function initPrayerEngine() {
    ...
    if (prayerTabContainer && !document.getElementById('adhan-volume-ctrl')) {
        ...
        adhanCtrlBtn.innerText = '🔊 صوت الأذان: مفعل';
...
// 3. دالة منفصلة لتحديث اتجاه القبلة والبوصلة
    ...
    if (window.DeviceOrientationEvent) {
        const handleOrientation = (event) => {
            ...
            if (event.webkitCompassHeading) {
                alpha = event.webkitCompassHeading; // دعم نظام iOS
            }
            if (alpha !== null && alpha !== undefined) {
                let compassRotation = qiblaAngle - alpha;
                qiblaCompass.style.transform = `rotate(${compassRotation}deg)`;
            }
        };

        if (typeof DeviceOrientationEvent.requestPermission === 'function') {
            qiblaCompass.addEventListener('click', () => {
                DeviceOrientationEvent.requestPermission()
                    .then(permissionState => {
                        if (permissionState === 'granted') {
                            window.addEventListener('deviceorientation', handleOrientation, true);
                            if (qiblaText) qiblaText.innerText = `زاوية القبلة: ${qiblaAngle.toFixed(2)} درجة (تم تفعيل المستشعر)`;
                        }
                    }).catch(console.error);
            }, { once: true });
            if (qiblaText) qiblaText.innerText = `زاوية القبلة: ${qiblaAngle.toFixed(2)} درجة (اضغط على البوصلة لتفعيلها في iOS)`;
        } else {
...
function startCountdown(timings) {
    ...
    countdownInterval = setInterval(() => {
        ...
        // 4. ضبط دقة توقيت الأذان: ينطلق فوراً عند تحقق دخول الوقت تماماً
        if (diffMinutes === 0 && secs === 0) { 
