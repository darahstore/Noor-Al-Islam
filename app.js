...
// ==========================================
// 5. محرك المسبحة والتخصيصات (Sebha & Themes) - ضبط منطق Pro
// ==========================================
const DhikrList = [
    "سُبْحَانَ اللَّهِ",
    "الْحَمْدُ لِلَّهِ",
...
// دالة محاكاة الإعلان التجريبي (تستقبل skipConfirm لتجنب مربع حوار مزدوج)
function simulateRewardAd(callback, skipConfirm = false) {
    if (!skipConfirm) {
        ...
        // تخطي التأكيد (للاستخدام مع الثيمات)
        setTimeout(() => {
            alert("✅ تم فتح الثيم مؤقتاً! شكراً لمشاهدتك الإعلان.");
            if (callback) callback();
        }, 1500);
    }
}
...
function initSebhaEngine() {
    ...
    // ---- تغيير الثيم مع التحقق من Pro وتجربة إعلان ----
    themeSelect.addEventListener('change', (e) => {
        const selectedTheme = e.target.value;
        const previousTheme = StorageManager.state.sebhaTheme;

        // إذا كان الثيم المحدد هو royal أو diamond
        if (selectedTheme === 'royal' || selectedTheme === 'diamond') {
            // إذا كان المستخدم Pro، طبق مباشرة
            if (isProUser) {
            ...
            // غير Pro: إيقاف التغيير مؤقتاً وإظهار رسالة
            themeSelect.value = previousTheme; // نعيد الثيم السابق في القائمة

            const message = "هذا الثيم متاح في النسخة المدفوعة، يمكنك مشاهدة إعلان قصير لفتحه الآن";
            if (confirm(message)) {
                // استدعاء الإعلان مع skipConfirm = true لتجنب مربع حوار مزدوج
                simulateRewardAd(() => {
                    // بعد الإعلان، نطبق الثيم
                    themeSelect.value = selectedTheme;
                    applySebhaTheme(selectedTheme);
                    StorageManager.update('sebhaTheme', selectedTheme);
                }, true);
            } else {
                // رفض المستخدم، يبقى على الثيم السابق ولا شيء آخر
            }
        } else {
            // الثيمات المجانية (default, wood) تطبق فوراً
            applySebhaTheme(selectedTheme);
            StorageManager.update('sebhaTheme', selectedTheme);
        }
    });
