...
// تم تعديله ليشمل:
// - ترتيب إلزامي: قارئ -> رواية -> سورة مع تعطيل القوائم التالية
// - عرض الروايات الخاصة بالقارئ فقط
// - معالجة أخطاء وفحص آمن
// ==========================================
let allReciters = [];
let currentAudioUrl = "";
...
function initAudioEngine() {
    ...
    // بداية: تعطيل قائمتي الرواية والسور حتى يتم اختيار القارئ
    ...
    selectReciter.addEventListener('change', (e) => {
        ...
        // تفعيل قائمة الرواية وإضافة خيارات القارئ فقط
        ...
        // السور تبقى معطلة حتى اختيار رواية
        selectSurah.disabled = true;
        selectSurah.title = "الرجاء اختيار الرواية أولاً";
    });
...
// دالة محاكاة الإعلان التجريبي
function simulateRewardAd(callback) {
    if (confirm("📢 لمشاهدة إعلان قصير لفتح الميزة؟ (محاكاة)")) {
        ...
        alert("يمكنك الترقية إلى Pro للاستمتاع بكل الميزات دون إعلانات.");
    }
}

function initSebhaEngine() {
    // استخدام العداد الرقمي الجديد
    ...
    // ---------- التعديلات الجديدة ----------
    // تخزين مؤقت للثيمات التي تم فتحها عبر الإعلان
    let tempUnlocked = JSON.parse(localStorage.getItem('tempUnlockedThemes') || '[]');

    // دالة لتحديث نصوص الخيارات مع إضافة القفل 🔒
    function updateThemeOptions() {
        const options = themeSelect.options;
        for (let opt of options) {
            const val = opt.value;
            if (val === 'royal' || val === 'diamond') {
                // النص الأساسي
                let baseText = val === 'royal' ? 'ذهبي' : 'ألماسي';
                // إضافة القفل إذا كان مقفلاً
                if (!isProUser && !tempUnlocked.includes(val)) {
                    opt.text = baseText + ' 🔒';
                } else {
                    opt.text = baseText;
                }
    ...
    // التنقل بين الأذكار (باستخدام الأزرار القديمة)
    ...
    // تغيير الثيم مع التحقق من Pro أو التخزين المؤقت
    themeSelect.addEventListener('change', (e) => {
        const theme = e.target.value;
        // الثيمات المدفوعة: royal, diamond
        if ((theme === 'royal' || theme === 'diamond') && !isProUser && !tempUnlocked.includes(theme)) {
            // إرجاع الثيم السابق
            themeSelect.value = StorageManager.state.sebhaTheme || 'default';
            simulateRewardAd(() => {
                // بعد الإعلان نفتح الثيم مؤقتاً
                if (!tempUnlocked.includes(theme)) {
                    tempUnlocked.push(theme);
                    localStorage.setItem('tempUnlockedThemes', JSON.stringify(tempUnlocked));
                }
                themeSelect.value = theme;
                applySebhaTheme(theme);
                StorageManager.update('sebhaTheme', theme);
                updateThemeOptions(); // تحديث واجهة القفل
            });
            return;
        }
        ...
        applySebhaTheme(theme);
        StorageManager.update('sebhaTheme', theme);
        updateThemeOptions(); // تحديث القفل بعد التغيير (مثلاً إذا أصبح Pro)
    });
}
...
function applySebhaTheme(theme) {
    ...
    // تحديث الجسم للثيم الملكي العام (إذا أردنا التأثير الشامل)
...
function initProFeatures() {
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', () => {
            ...
            if (isProUser) {
                ...
                // عرض خيار الإعلان أو الترقية
                if (confirm("التحميل ميزة Pro. هل ترغب بمشاهدة إعلان لتحميل هذه السورة؟")) {
    ...
    // ربط أزرار التبرع والاشتراك في النافذة
    if (donateModal) {
        // زر الاشتراك الشهري Pro (المميز)
        ...
        donateBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // يمكن فتح Pro تلقائياً للدعم السخي إذا أردنا
                if (amount === '5') {
                    activatePro();
                }
                donateModal.classList.add('hidden');
...
function startDownload(url) {
    ...
    // تحديث واجهة الزر ليشير إلى اكتمال التحميل
    ...
    setTimeout(() => {
        if (downloadIcon) downloadIcon.innerText = '🔒';
        if (downloadText) downloadText.innerText = 'تحميل السورة';
    }, 2500);
}
...
function updateProUI() {
    // يمكن إخفاء أقفال أو تغيير شكل الأزرار بناءً على حالة Pro
    const downloadIcon = document.getElementById('download-icon');
    if (downloadIcon) {
